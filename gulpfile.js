...
gulp.task('build', series(clean, parallel(series(styles, minifyStyles), html, images)));
gulp.task('default', series('build', watch));
gulp.task('deploy', series('build', deploy));
...
function html() {
    let aConfig = config.htmlAssets,
        files = aConfig.sourceFiles,
        dest = aConfig.destinationDir;

    return copy(files, dest);
}
html.description = 'Copy html files from the app into the build folder';
...
function deploy(done) {
    surge({
        project: config.buildDir
    });
    done();
}
deploy.description = 'Deploy the built application to Surge.sh';
...
function watch(done) {
    if (development()) {
        ...
        gulp.watch(config.htmlAssets.sourceFiles, html);
